...
import { Button, Form, Input, Select, FormItemProps, DatePickerProps, DatePicker } from "antd";
...
const FilterForm = React.memo(
  withFallback(
    ...
    ({ filterItems, onFilter, onAdd, initialValues, styles }: FilterFormProps<T>) => {
    const [form] = Form.useForm();

    const handleFilter = () => {
      startTransition(() => {
        const values = form.getFieldsValue();
        onFilter(values as T);
      });
    };
      ...
      switch (item.type) {
        case 'input':
          return (
            ...
            >
              <Input placeholder={item.placeholder} allowClear style={item.style} />
            </Form.Item>
          );
        case 'select':
          return (
        ...
        case 'booleanSelect':
          ...
          case 'rangePicker':
            const placeholder: [string, string] | undefined = Array.isArray(item.placeholder)
            ? item.placeholder
            : ['开始日期', '结束日期'];
            return (
              <Form.Item
